...
ggplot(data = tests) +
  ...
  ggfx::with_shadow(
    geom_errorbarh(mapping = aes(y = He, xmin = Zeit, xmax = Bis), height = .02, size = 1.5, color = "mediumslateblue"),
    color = "navy", x_offset = 0, y_offset = 4, sigma = 1) +
  geom_label(x = frame[2], y = .85, label = strftime(x = frame[2], format = "%d. %B\n%H:%M"),
